...
export const selectProductById = (id?: string | null) =>
  createSelector([selectProducts], (items) => items.find((p) => p._id === id) || null);

export const selectIngredientsByIds = (ids: string[]) =>
  createSelector([selectIngredients], (all) => all.filter((i) => ids.includes(i._id)));
...
export const selectTotalPrice = (
  ...
  createSelector(
    [selectProductById(productId), selectIngredients],
    (product, allIngs) => {
      if (!product) return 0;
